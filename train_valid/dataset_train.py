...
#
#             # 0.05 时0.27
#             loss = alpha * (ori_loss + au_weight * au_loss + mix_w * mix_loss) + (1 - alpha) * re_loss
#             loss.backward()
#             optimize.step()
#
#             avg_loss += loss.item()
...
#
#
# def dataset_train_limix_rspace_cnn(model, token, data, criterion, optimize, n_epoch, au_weight, device, mix_w, scheduler=None,
#                              model_path=None, mix_alpha=1.):
#     print("limix_rspace".center(30, "*"))
#     model.to(device=device)
#     best_val_f1, counts, tmp, best_epoch = 0, 0, 0, 0
...
def dataset_train_rdrop(model, token, data, criterion, optimize, n_epoch, au_weight, device, scheduler=None, model_path=None):
    model.to(device=device)
    best_val_f1, counts, tmp, best_epoch = 0, 0, 0, 0
    train_sen = data['train']['sen']
    train_tar = data['train']['tar']
    ...
    for i in range(1, n_epoch + 1):
        ...
        for index, (t_sen, t_tar, r_sen, r_tar, s_sen, s_tar) in enumerate(zip(train_sen, train_tar, re_sen, re_tar,
            ...
            ori_loss = 0.5 * (ori_loss1 + ori_loss2)

            ori_loss = ori_loss + ori_kl * 4

            re_kl = compute_kl_loss(p=au_output1, q=au_output2)

            # L_r
            re_loss1 = criterion(main_output, train_r_tar.to(device))
            re_loss2 = criterion(main_output, train_r_tar.to(device))
...
def dataset_train_suploss(model, token, data, criterion, optimize, n_epoch, au_weight, device, scheduler=None, model_path=None):
    model.to(device=device)
    best_val_f1, counts, tmp, best_epoch = 0, 0, 0, 0
    train_sen = data['train']['sen']
    train_tar = data['train']['tar']
...
# def dataset_train_paper_num(model, token, data, criterion, optimize, n_epoch, au_weight, device, scheduler=None, model_path=None):
#     model.to(device=device)
#     best_val_f1, counts, tmp, best_epoch = 0, 0, 0, 0
#     train_sen = data['train']['sen']
#     train_tar = data['train']['tar']
#     train_num = data['train']['num']
#     re_sen = data['reverse']['sen']
#     re_tar = data['reverse']['tar']
#     re_num = data['reverse']['num']
#     sec_sen = data['section']['sen']
#     sec_tar = data['section']['tar']
#     assert len(train_sen) <= len(sec_sen)
#
...
#         tst = time.time()
#
#         for index, (t_sen, t_tar, t_num, r_sen, r_tar, r_num, s_sen, s_tar) in enumerate(zip(train_sen, train_tar, train_num,
...
#             r_label2one = F.one_hot(r_num - 1, 14).float().to(device)
#             optimize.zero_grad()
#
#             t_sent = t_sent.to(device)
#             r_sent = r_sent.to(device)
...
#             train_r_tar = torch.LongTensor(r_tar)
#             s_tar = torch.LongTensor(s_tar)
#             main_output, au_output1 = model(t_sent, r_sen=r_sent, s_sen=s_sent, l=alpha, t_one=t_label2one, r_one=r_label2one)
#             # L_o
#             ori_loss = criterion(main_output, train_t_tar.to(device))
#             # L_r
#             re_loss = criterion(main_output, train_r_tar.to(device))
