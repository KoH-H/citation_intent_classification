...
def supcnn(model, token, data, criterion, optimize, n_epoch, au_weight, device, scheduler=None, model_path=None):
    ...
    for i in range(1, n_epoch + 1):
        ...
        for index, (t_sen, t_tar, r_sen, r_tar, s_sen, s_tar) in enumerate(zip(train_sen, train_tar, re_sen, re_tar,
            ...
            # L_o
            ori_loss = criterion(main_output, train_t_tar.to(device)) + osuploss
            # L_r
...
def onlysup(model, token, data, criterion, optimize, n_epoch, au_weight, device, scheduler=None, model_path=None):
    model.to(device=device)
    best_val_f1, counts, tmp, best_epoch = 0, 0, 0, 0
    train_sen = data['train']['sen']
    train_tar = data['train']['tar']
...
def onlycnn(model, token, data, criterion, optimize, n_epoch, au_weight, device, scheduler=None, model_path=None):
    model.to(device=device)
    best_val_f1, counts, tmp, best_epoch = 0, 0, 0, 0
    train_sen = data['train']['sen']
    train_tar = data['train']['tar']
